...
def index(request):
    data = {
        ...
        'destinationData': Destination.objects.filter(category="8") &
                           Destination.objects.filter(popular=1) &
                           Destination.objects.filter(status=1),
        'destinationDataNepal': Destination.objects.filter(category="9") &
                                Destination.objects.filter(popular=1) &
                                Destination.objects.filter(status=1),
...
def destination(request):
    destination_list = Destination.objects.filter(status=1).order_by('price')
...
def destination_details(request, criteria):
    ...
    if request.method == 'POST' and 'booking' in request.POST:
        ...
        if form.is_valid():
            ...
            message = request.POST.get('message')
            if int(person) < destinationdetails.stock:
                destinationdetails.stock -= int(person)
                ...
                messages.error(request, 'Ordered quantity is greater than stock.',
            ...
            # text_content = render_to_string('inventory_booking_email_template/email.txt', details)
            # html_content = render_to_string('inventory_booking_email_template/email.html', details)
